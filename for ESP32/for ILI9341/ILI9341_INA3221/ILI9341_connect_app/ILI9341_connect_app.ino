...
void setup() {
  ...
  rtc.setTime(00, 23, 1, 21, 10, 2022);
...
void showbattery(void) {
  ...
  if (numloop%10 == 0){
    ...
    if  (WiFi.status() == WL_CONNECTED){
      server.on("/battper", HTTP_GET, [](AsyncWebServerRequest *request){
          request->send(200, "text/plain", String(battery1));
 });
    }
  }
}
...
void showtemp(void) {
  ...
  if (WiFi.status() == WL_CONNECTED){
     ...
     server.on("/tempf", HTTP_GET, [](AsyncWebServerRequest *request){
      request->send(200, "text/plain", String(tempF));
 });
  }
  delay(200);
}
